...
import { ProgAndTestTask, ProgTask, TestTask } from 'entities/Candidate/TestTask'
...
export const testUserSlice = createSlice({
  ...
  reducers: {
    ...
    setStatusTask: (
      state,
      action: PayloadAction<{ idItem: number | string; status: StatusItemTest; task: ProgTask | TestTask }>
    ) => {
      state.data.statusItem[action.payload.idItem] = { status: action.payload.status, id: action.payload.task.id }
    },
